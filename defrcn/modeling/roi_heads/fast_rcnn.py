...
def get_semantic_vectors(class_names, norm=True):
    ...
    with torch.no_grad():
        ...
        for class_name in class_names:
            text_features.append(np.loadtxt('datasets/clip/' + class_name + '.txt'))
        text_features.append(np.loadtxt('datasets/clip/' + 'background.txt'))
...
class AuxFastRCNNOutputs(FastRCNNOutputs):
    """
    A class that stores information about outputs of a Fast R-CNN head.
    """
    ...
    def __init__(
        ...
        self.aux_pred_class_logits = aux_pred_class_logits
        self.inference_with_aux = inference_with_aux
    ...
    def aux_softmax_cross_entropy_loss(self):
        """
        Compute the softmax cross entropy loss for box classification.
        ...
        self._log_accuracy()
        return F.cross_entropy(
            self.aux_pred_class_logits, self.gt_classes, reduction="mean"
        )
    ...
    def losses(self):
        ...
        """
        loss_dict = {
            "loss_cls": self.weight_ce * self.softmax_cross_entropy_loss(),
            "loss_aux_cls": self.aux_softmax_cross_entropy_loss(),
            "loss_box_reg": self.smooth_l1_loss(),
        }

        loss_dict["loss_kd"] = (self.weight_kd / self.pred_class_logits.shape[1]) \
                               * dandr_loss(self.pred_class_logits, self.aux_pred_class_logits,
                                           self.gt_classes, self.kd_alpha, self.kd_beta,
                                               self.kd_temperature)
        return loss_dict
    ...
    def predict_probs(self):
        ...
        Returns:
            list[Tensor]: A list of Tensors of predicted class probabilities for each image.
                ...
                for image i.
        """
        if self.inference_with_aux:
            probs = F.softmax(self.aux_pred_class_logits, dim=-1)
        else:
            probs = F.softmax(self.pred_class_logits, dim=-1)
        return probs.split(self.num_preds_per_image, dim=0)
...
@ROI_HEADS_OUTPUT_REGISTRY.register()
class AuxFastRCNNOutputLayers(FastRCNNOutputLayers):
    """
    Two linear layers for predicting Fast R-CNN outputs:
      (1) proposal-to-detection box regression deltas
      (2) classification scores
    ...
    def __init__(
        ...
        Args:
            ...
            box_dim (int): the dimension of bounding boxes.
                Example box dimensions: 4 for regular XYXY boxes and 5 for rotated XYWHA boxes
        """
        super(AuxFastRCNNOutputLayers, self).__init__(
            cfg=cfg,
            input_size=input_size,
            num_classes=num_classes,
            cls_agnostic_bbox_reg=cls_agnostic_bbox_reg,
            box_dim=box_dim)
        semantic_dim = cfg.AUX_MODEL.SEMANTIC_DIM
    ...
    def forward(self, x):
        ...
        return scores, aux_cls_score, proposal_deltas