...
from .fast_rcnn import ROI_HEADS_OUTPUT_REGISTRY, FastRCNNOutputLayers, \
    FastRCNNOutputs, AuxFastRCNNOutputs
...
class Res5ROIHeads(ROIHeads):
    ...
    def forward(self, images, features, proposals, targets=None, inference_with_aux=False):
        """
...
@ROI_HEADS_REGISTRY.register()
class AuxRes5ROIHeads(Res5ROIHeads):
    """
    The ROIHeads in a typical "C4" R-CNN model, where the heads share the
    cropping and the per-region feature computation by a Res5 block.
    """
    ...
    def __init__(self, cfg, input_shape):
        super().__init__(cfg, input_shape)
        self.kd_temperature = cfg.AUX_MODEL.KD_TEMPERATURE
        self.inference_with_aux = cfg.AUX_MODEL.INFERENCE_WITH_AUX
        self.weight_ce = cfg.AUX_MODEL.WEIGHT_CE
        self.weight_kd = cfg.AUX_MODEL.WEIGHT_KD
        self.kd_alpha = cfg.AUX_MODEL.KD_ALPHA
        self.kd_beta = cfg.AUX_MODEL.KD_BETA
    ...
    def forward(self, images, features, proposals, targets=None, inference_with_aux=False):
        ...
        feature_pooled = box_features.mean(dim=[2, 3])  # pooled to 1x1
        pred_class_logits, aux_pred_class_logits, pred_proposal_deltas = self.box_predictor(
            feature_pooled
        )
        del feature_pooled

        inference_with_aux = inference_with_aux or self.inference_with_aux
        outputs = AuxFastRCNNOutputs(
            self.box2box_transform,
            pred_class_logits,
