...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs, inference_with_aux=False):
        if not self.training:
            return self.inference(batched_inputs, inference_with_aux=inference_with_aux)
    ...
    def inference(self, batched_inputs, inference_with_aux):
        assert not self.training
        _, _, results, image_sizes = self._forward_once_(batched_inputs, None, inference_with_aux=inference_with_aux)
        processed_results = []
    ...
    def _forward_once_(self, batched_inputs, gt_instances=None, inference_with_aux=False):
