...
class DefaultTrainer(SimpleTrainer):
    ...
    def build_hooks(self):
        ...
        def test_and_save_results():
            import copy
            proto_dataset = copy.deepcopy(self.data_loader.proto_dataset)
            self._last_eval_results = self.test(self.cfg, self.model, proto_dataset=proto_dataset)
            return self._last_eval_results

        def test_and_save_aux_results():
            import copy
            proto_dataset = copy.deepcopy(self.data_loader.proto_dataset)
            self._last_eval_results = self.test(self.cfg, self.model, proto_dataset=proto_dataset, inference_with_aux=True)
            return self._last_eval_results
        ...
        ret.append(EvalHookDeFRCN(
            cfg.TEST.EVAL_PERIOD, test_and_save_aux_results, self.cfg))
    ...
    @classmethod
    def test(cls, cfg, model, evaluators=None, inference_with_aux=False, proto_dataset=None):
        """
        Args:
            cfg (CfgNode):
            model (nn.Module):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            results_i = inference_on_dataset(model, data_loader, evaluator, cfg,
                                             inference_with_aux=inference_with_aux,
